...
from mcpsec.sql_scanner.detector import SQLIFinding, SQLInjectionDetector
...
class SQLScanner:
    ...
    async def scan_server(
        self, profile, level: int = 1, fingerprint: bool = False
    ) -> list[SQLIFinding]:
        ...
        for tool in profile.tools:
            ...
            for finding in tool_findings:
                ...
                if await self.confirmer.confirm(finding):
                    ...
                    if fingerprint:
                        db_type = await self.fingerprinter.fingerprint(
                            finding.tool, finding.parameter, finding.payload
                        )
