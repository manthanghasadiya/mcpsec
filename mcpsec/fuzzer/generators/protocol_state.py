...
INTENSITY_LEVELS = {"low": 10, "medium": 25, "high": 50, "insane": 50}

def generate(framing: str = "clrf", intensity: str = "medium") -> list[FuzzCase]:
    ...
    _add("full_lifecycle", lifecycle, "Complete init → list → call lifecycle")
    ...
    _add(
        "skip_to_prompts", _f(_rpc("prompts/list", req_id=1)), "prompts/list without initialization"
    )
    ...
    _add("reinit_after_list", reinit, "Re-initialize after tools/list")

    triple_init = b"".join(_f(_rpc("initialize", INIT_PARAMS, i)) for i in range(1, 4))
    _add("triple_init", triple_init, "Three initialize requests in a row")

    # ── Ping tests ───────────────────────────────────────────────────
    _add("ping_before_init", _f(_rpc("ping", req_id=1)), "Ping before initialization")
    ...
    _add("ping_after_init", ping_after, "Ping after proper initialization")

    rapid_pings = b"".join(_f(_rpc("ping", req_id=i)) for i in range(1, 101))
    _add("rapid_100_pings", rapid_pings, "100 pings sent rapidly")

    # ── Resources operations ─────────────────────────────────────────
    _add(
        "resources_read_no_uri",
    ...
    subscribe_unsubscribe = _f(_rpc("resources/subscribe", {"uri": "test://resource"}, 1)) + _f(
        _rpc("resources/unsubscribe", {"uri": "test://resource"}, 2)
    )
    _add("subscribe_unsubscribe", subscribe_unsubscribe, "Subscribe then immediately unsubscribe")

    _add(
        "unsubscribe_never_subscribed",
    ...
    for i in range(50):
        if i % 3 == 0:
            ...
            rapid_transitions += _f(
                _rpc("tools/call", {"name": f"tool_{i}", "arguments": {}}, i + 1000)
            )
        else:
            rapid_transitions += _f(_rpc("ping", req_id=i + 1000))
    _add("rapid_transitions_50", rapid_transitions, "50 rapid list/call/ping transitions")

    # ── Cancel with progressToken ────────────────────────────────────
    call_with_progress = _f(
        _rpc(
            "tools/call",
            {"name": "slow_tool", "arguments": {}, "_meta": {"progressToken": "xyz-123"}},
            1,
        )
    ) + _f(_notif("notifications/cancelled", {"requestId": 1, "reason": "timeout"}))
    _add("call_then_cancel", call_with_progress, "tools/call with progressToken then cancel")
    ...
    _add("batch_array", _f(batch), "JSON-RPC batch (array of requests) — not in MCP spec")
    ...
    for i, m in enumerate(internal_methods):
        _add(f"internal_method_{i}", _f(_rpc(m, req_id=i + 2000)), f"Internal-looking method: {m}")
    ...
    for msg in pre_init_methods:
        cases.append(
            FuzzCase(
                ...
                generator="protocol_state",
                payload=_f(json.dumps(msg).encode()),
    ...
    for name, version, desc in bad_versions:
        params = {"capabilities": {}, "clientInfo": {"name": "mcpsec-fuzzer", "version": "1.0.0"}}
        ...
        cases.append(
            FuzzCase(
                ...
                generator="protocol_state",
                payload=_f(json.dumps({"jsonrpc": "2.0", "id": 200, "method": "initialize", "params": params}).encode()),
                description=desc,
                expected_behavior="Server handles state correctly",
                expects_error=True,
                crash_indicates_bug=True,
    ...
    for name, payload, desc, skip_init in notification_cases:
        cases.append(
            FuzzCase(
                ...
                generator="protocol_state",
                payload=_f(json.dumps(payload).encode()),
                description=desc,
                expected_behavior="Server handles state correctly",
                expects_error=False,
                crash_indicates_bug=True,
    ...
    for method in post_shutdown_methods:
        cases.append(
            FuzzCase(
                ...
                generator="protocol_state",
                payload=_f(json.dumps({"jsonrpc": "2.0", "id": 500, "method": method, "params": {}}).encode()),
