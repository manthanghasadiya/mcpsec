...
def generate(framing: str = "clrf") -> list[FuzzCase]:
    ...
    if framing == "jsonl":
        _add("jsonl_no_newline", body, "JSONL message without trailing newline")
    ...
    _add("raw_bytes_no_frame", body, "Raw JSON body with no framing at all")
    _add(
        "double_frame",
        (f"Content-Length: {len(body)}\r\n\r\n".encode() + body)
        if framing == "clrf"
