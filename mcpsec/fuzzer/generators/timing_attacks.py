...
def generate(framing: str = "clrf") -> list[FuzzCase]:
    ...
    _add("burst_10_tools_list", burst_10, "10 tools/list requests in single write")

    burst_50 = b"".join(_f(_rpc("ping", req_id=i)) for i in range(1, 51))
    _add("burst_50_pings", burst_50, "50 ping requests in single write")

    burst_100 = b"".join(_f(_rpc("tools/list", req_id=i)) for i in range(1, 101))
    _add("burst_100_tools_list", burst_100, "100 tools/list requests in single write (flood)")
    ...
    _add("interleave_valid_invalid_20", interleaved, "20 alternating valid/invalid messages")

    # ── Interleaved methods ──────────────────────────────────────────
    methods = ["tools/list", "ping", "resources/list", "prompts/list"]
    mixed = b"".join(_f(_rpc(methods[i % len(methods)], req_id=i + 200)) for i in range(40))
    _add("mixed_methods_40", mixed, "40 rapid requests cycling through different methods")

    # ── Duplicate IDs (race) ─────────────────────────────────────────
    dup_ids = b"".join(_f(_rpc("tools/list", req_id=999)) for _ in range(10))
    _add("dup_id_race_10", dup_ids, "10 requests all with id=999 (ID collision race)")
    ...
    _add("request_notification_mix", req_notif, "Interleaved requests and cancel notifications")
    ...
    if framing == "clrf":
        ...
        _add("slow_loris_half", header + half, "Half of a message body (slow-loris)")
    else:
        _add("slow_loris_half", half, "Half of a message body (slow-loris)")

    # ── Partial header (CLRF only) ───────────────────────────────────
    if framing == "clrf":
        _add("partial_header", b"Content-Leng", "Truncated header (partial Content-Length)")
    ...
    _add(
        "single_byte_message",
        full_msg,  # Can't actually fragment in our engine
    ...
    _add("pipeline_30_methods", pipeline, "30 pipelined requests across 6 different methods")
    ...
    _add("massive_200_pings", massive, "200 pings in a single write (~100KB)")
    ...
    _add("rapid_call_20_nonexistent", calls, "20 rapid tools/call to nonexistent tools")
    ...
    _add("cancel_flood_50", cancels, "50 cancel notifications for non-existent requests")

    # ── Out-of-order IDs ─────────────────────────────────────────────
    ooo = b"".join(_f(_rpc("tools/list", req_id=rid)) for rid in [100, 3, 999, 1, 50, 7, 42])
    _add("out_of_order_ids", ooo, "Requests with non-sequential IDs")

    # ── Negative IDs ─────────────────────────────────────────────────
    neg = b"".join(_f(_rpc("tools/list", req_id=rid)) for rid in [-1, -100, -999999])
    _add("negative_ids", neg, "Requests with negative IDs")

    # ── Float IDs ────────────────────────────────────────────────────
    for fid in [1.5, 0.0, -0.0, 1e10, 1e-10]:
        msg = {"jsonrpc": "2.0", "method": "tools/list", "id": fid}
        _add(
            f"float_id_{str(fid).replace('.', '_').replace('-', 'neg')}",
