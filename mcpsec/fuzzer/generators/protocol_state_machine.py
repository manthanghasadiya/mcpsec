...
def generate(intensity: str = "medium") -> list[dict[str, Any]]:
    ...
    malformed_inits = [
        ...
        (
            "null_capabilities",
            {"protocolVersion": "2024-11-05", "capabilities": None},
            "Null capabilities",
        ),
        (
            "string_capabilities",
        ...
        (
            "null_client_info",
            {"protocolVersion": "2024-11-05", "capabilities": {}, "clientInfo": None},
            "Null clientInfo",
        ),
        (
            "empty_client_info",
    ...
    notification_cases = [
        ...
        (
            "premature_initialized",
            {"jsonrpc": "2.0", "method": "notifications/initialized"},
            "Send initialized notification before initialize request",
            True,
        ),
        # initialized with ID (should be notification, no ID)
        (
            "initialized_with_id",
            {"jsonrpc": "2.0", "id": 400, "method": "notifications/initialized"},
            "initialized notification with ID (invalid)",
            False,
        ),
        ...
        (
            "double_initialized",
            {"jsonrpc": "2.0", "method": "notifications/initialized"},
            "Send initialized notification twice",
            False,
        ),
        # cancelled notification for non-existent request
        (
            "cancel_nonexistent",
            {"jsonrpc": "2.0", "method": "notifications/cancelled", "params": {"requestId": 99999}},
            "Cancel non-existent request",
            False,
        ),
        # progress notification for non-existent request
        (
            "progress_nonexistent",
            {
                "jsonrpc": "2.0",
                "method": "notifications/progress",
                "params": {"progressToken": "fake", "progress": 50},
            },
            "Progress for non-existent request",
            False,
        ),
    ]
