...
from .chain_builder import AttackChain, ChainBuilder
...
class ChainExecutionResult:
    ...
    def is_finding(self) -> bool:
        ...
        return (
            self.crash_detected
            or self.exploitation_evidence is not None
            or self.status == ChainExecutionStatus.CRASH_DETECTED
        )
...
class ChainEngine:
    ...
    async def run(
        ...
        try:
            ...
            if not self._tools:
                console.print(
                    "[yellow]No tools discovered. Cannot perform chained fuzzing.[/yellow]"
                )
                return []
            ...
            console.print(
                f"  Found [cyan]{len(injection_points)}[/cyan] potential injection points"
            )
    ...
    async def _execute_all_chains(self) -> None:
        ...
        with Progress(
            ...
            task = progress.add_task("Executing chains...", total=len(self._chains))
    ...
    async def _execute_single_chain(self, chain: AttackChain, payload: str) -> ChainExecutionResult:
        ...
        try:
            ...
            for i, setup_step in enumerate(chain.setup_steps):
                ...
                if response is None:
                    ...
                    result.error_message = (
                        f"Server crashed during setup step: {setup_step.tool_name}"
                    )
                    return result
                ...
                if "error" in response:
                    ...
                    result.error_message = (
                        f"Setup failed at {setup_step.tool_name}: {response.get('error')}"
                    )
                    return result
            ...
            if self.config.verbose:
                console.print(
                    f"    [dim]Target: {chain.target_tool}({chain.injection_point}={payload[:30]}...)[/dim]"
                )
    ...
    def _resolve_arguments(
        self, args_template: dict[str, Any], state: StateManager
    ) -> dict[str, Any]:
    ...
    def _inject_payload(
        self, args: dict[str, Any], injection_point: str, payload: str
    ) -> dict[str, Any]:
    ...
    def _check_exploitation_evidence(
        ...
        is_error_response = "error" in response_lower or any(
            p in response_lower for p in error_patterns
        )
    ...
    def _extract_evidence_context(
        self, response: str, indicator: str, context_chars: int = 150
    ) -> str:
    ...
    def _print_results_summary(self) -> None:
        ...
        for finding in findings:
            table.add_row(
                ...
                finding.payload_used[:30] + "..."
                if len(finding.payload_used) > 30
                else finding.payload_used,
                (finding.exploitation_evidence[:40] + "...")
                if finding.exploitation_evidence
                else "Crash",
            )
