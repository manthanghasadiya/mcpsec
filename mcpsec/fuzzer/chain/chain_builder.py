...
class ChainBuilder:
            ...
            for injection_point in tool_injection_points:
                ...
                if requires_state:
                    for state_key in requires_state:
                        provider_tool = self._find_state_provider(
                            state_key, dependency_graph, tools_by_name
                        )
                        if provider_tool:
                            ...
                            if provider_dep and provider_dep.requires:
                                ...
                                for sub_req in provider_dep.requires:
                                    sub_provider = self._find_state_provider(
                                        sub_req, dependency_graph, tools_by_name
                                    )
                                    if sub_provider and sub_provider != provider_tool:
                                        setup_steps.append(
                                            ChainStep(
                                                tool_name=sub_provider,
                                                arguments=self._create_argument_template(
                                                    tools_by_name[sub_provider],
                                                    dependency_graph.get(sub_provider),
                                                ),
                                                purpose="setup",
                            ...
                            setup_steps.append(
                                ChainStep(
                                    ...
                                    arguments=self._create_argument_template(
                                        tools_by_name[provider_tool], provider_dep
                                    ),
    ...
    def _find_state_provider(
        ...
        for tool_name, dep in dependency_graph.items():
            ...
            if state_key in ["ref", "element", "id", "node"] and any(
                p in ["ref", "id", "node", "element"] for p in provides
            ):
                return tool_name
        return None

    def _create_argument_template(self, tool: dict, dep: ToolDependency | None) -> dict[str, Any]:
