...
class DescriptionPromptInjectionScanner(BaseScanner):
    ...
    INJECTION_PATTERNS = [
        ...
        (r"you\s+are\s+now\s+(a|an|acting\s+as)", "Role reassignment attempt", Severity.HIGH),
        (
            r"new\s+(instructions?|rules?|prompt)\s*:",
            "New instruction injection",
            Severity.CRITICAL,
        ),
        (r"system\s*:\s*", "System prompt injection", Severity.CRITICAL),
        (r"<\s*system\s*>", "XML system tag injection", Severity.CRITICAL),
        (r"</?\s*(?:admin|root|sudo|system)\s*>", "Privilege tag injection", Severity.HIGH),
        ...
        (r"run\s+(this|the\s+following|these)", "Run instruction", Severity.MEDIUM),
        (r"always\s+(call|use|execute|run)\s+this\s+tool", "Forced tool execution", Severity.HIGH),
        (
        ...
        (r"hidden\s+instruction", "Explicit hidden instruction", Severity.CRITICAL),
        (
        ...
        (
            ...
            "Data forwarding instruction",
            Severity.HIGH,
        ),
        # Jailbreak patterns
        (r"\b(DAN|STAN|DUDE|AIM)\b\s*(mode|prompt)?", "Known jailbreak pattern", Severity.HIGH),
        (
            r"pretend\s+(you\s+)?(are|can|have|don'?t)",
            "Pretend/roleplay manipulation",
            Severity.MEDIUM,
        ),
        (r"act\s+as\s+(if|though)\s+you", "Behavior manipulation", Severity.MEDIUM),
    ]

    # Unicode tricks for hiding text
    UNICODE_TRICKS = [
        (r"[\u200b\u200c\u200d\u2060\ufeff]", "Zero-width characters (hidden text)", Severity.HIGH),
        (
            r"[\u202a-\u202e\u2066-\u2069]",
            "Bidirectional text override (text hiding)",
            Severity.HIGH,
        ),
        (r"[\u0000-\u001f\u007f-\u009f]", "Control characters", Severity.MEDIUM),
        (r"[\ue000-\uf8ff]", "Private use area characters", Severity.LOW),
        (
    ...
    def _scan_schema(self, schema: dict, item_type: str, item_name: str) -> list[Finding]:
        ...
        if isinstance(schema, dict):
            ...
            for prop_name, prop_schema in schema.get("properties", {}).items():
                findings.extend(
                    self._scan_schema(
                        prop_schema, f"{item_type}_property", f"{item_name}.{prop_name}"
                    )
                )
