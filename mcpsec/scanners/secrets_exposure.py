...
SECRET_PATTERNS = [
    ...
    # Reading .env files
    (r"""(?:readFile|readFileSync|open)\s*\(.*\.env""", "Reading .env File", Severity.MEDIUM),
...
def should_skip_file(file_path: Path) -> bool:
    ...
    if any(
        name_upper.startswith(x)
        for x in ["README", "CHANGELOG", "LICENSE", "CONTRIBUTING", "HISTORY"]
    ):
        return True
            ...
            if re.search(pattern, stripped, re.IGNORECASE):
                findings.append(
                    Finding(
                        ...
                        description="Potential secret or credential exposure detected.",
